...
### Subtasks
- [ ] 1.1 Replace `storage_dirs` in `metal/roles/storage/vars/main.yml` with the designed tree as a flat path list (top-level dirs, `NN.MM` subdirs, `rotation/{movies,shows}`, `downloads/{complete,incomplete}`, `10_documents/inbox`)
...
### Subtasks
- [ ] 2.1 Add `exclude rotation/`, `exclude inbox/`, `exclude /99_tmp/` (root-anchored) with why-comments to `metal/roles/storage/templates/snapraid.conf.j2`
...
- [ ] 5.2 Rework `persistence` in `apps/jellyfin/values.yaml`: single NFS claim with subPath advancedMounts per container (Jellyfin all-`readOnly`; transmission `/downloads`; radarr `/movies`→`rotation/movies`; sonarr `/shows`→`rotation/shows`; identical `/downloads` paths everywhere); strip media subPaths from the Ceph `data` PVC (configs only)
- [ ] 5.3 Update transmission download-dir config to `/downloads/complete` + `/downloads/incomplete`
- [ ] 5.4 Update `docs/guides/how_to_for_media_management.md` (root folders, Jellyfin library paths for both tiers)
- [ ] 5.5 Verify: helm template shows single-claim mounts with correct RO flags; in-cluster hardlink proof (`ln` across `/downloads`→`/movies`, `stat -c %h` = 2); EROFS on write in Jellyfin's `/media/movies`; `./tests/metal.sh` passes
...
- [ ] 6.2 Generate b3sum manifests: NAS copy, drive B, drive C → `00_meta/migration/*.b3`
...
- [ ] 7.3 Exit check: `99_tmp` empty; sampled `b3sum -c` audit of moved files against `moves.log`
...
### Subtasks
- [ ] 8.1 Wipe drive B, add to `parity_drives` in sops inventory, run storage role; initial sync completes (`journalctl -u snapraid-initial-sync`)
...
- **Depends on:** Task 1
- **Size:** S
- **Can run in parallel with:** Task 4, Task 5, Task 6, Task 7
