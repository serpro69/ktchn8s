...
- [ ] NFS branch review follow-ups
    ...
    - [x] **Create the `nfs-csi-videos` StorageClass or drop the reference** — resolved with the PV move: `storageClassName` was dropped from the PV entirely and the PVC binds statically via `volumeName` + `storageClassName: ""`; no phantom class remains.
