...
- [ ] NFS branch review follow-ups
    Non-blocking findings from the pre-merge code review of the `nfs` branch (2026-04-21, review doc was local-only).
    Deferred as non-blocking for the merge (no correctness findings), but the stack is **untested end-to-end**: NAS-side ansible was applied ~Feb 2026 (pre-dating the Feb 21 storage-firewall and Mar 25 router-ACL commits), and the K8s side (csi-driver-nfs, PV/PVC binding, Jellyfin NFS mount) has never run — the nodes have been powered off since. First boot after the merge is the real validation: rerun `make metal` to reconcile NAS drift, then `./tests/metal.sh` and the storage role's NFS validation tasks.
    - [ ] **Move the Videos PV out of the Jellyfin chart** — `apps/jellyfin/templates/pv-videos.yaml` is a cluster-scoped resource inside an app chart; Helm couples its lifecycle to the app and re-installs risk orphaned PV objects. Lift the PV into a `system/`-level chart (e.g. alongside `csi-driver-nfs`), keep only the PVC in the app chart — or switch to dynamic provisioning via a `StorageClass`. This pattern will repeat for every future NFS share (Pictures, Documents, Music, Backups), so fix it once before adding more shares.
    - [ ] **Replace hardcoded NAS IP with a DNS name** — `apps/jellyfin/values.yaml` uses `10.10.10.30` directly; breaks silently if the NAS IP ever changes. Add a DNS record for the NAS and reference it by name.
    ...
    - **Helm chart pinning — decision (2026-08-18): keep the status quo.** Exact version pins in `Chart.yaml` + daily Renovate bumps; no `Chart.lock`, no vendored `charts/*.tgz` (both stay gitignored). Rationale: with exact pins, `Chart.lock` adds nothing — its digest hashes dependency *metadata*, not tarball content, so it verifies neither integrity nor reproducibility. Vendoring archives was considered and rejected: with the cluster running daily, chart repos are only needed at sync/render time, and a failed render leaves the last-synced state running — upstream outages delay updates rather than break workloads. Accepted residual risk: a full cluster rebuild while an upstream repo is down or has pruned the pinned version requires bumping to an available version first. Future code reviews should not re-flag missing `Chart.lock` files.
    - [ ] **Create the `nfs-csi-videos` StorageClass or drop the reference** — the PV sets `storageClassName: nfs-csi-videos` but the csi-driver-nfs chart has `storageClass.create: false`, so the class never exists; static PV/PVC binding works regardless, but the dangling name is misleading.
    - [ ] **Protect `smtp_password` from leaking** — `metal/roles/storage/defaults/main.yml` relies on a "use vault!" comment only. Add `no_log: true` to the msmtp template task and document ansible-vault usage in the role.
    - [ ] Minor cleanups from the same review:
        ...
        - `apps/jellyfin/templates/pv-videos.yaml` `volumeHandle` embeds a `/share/` path segment that maps to nothing real (copy-paste leftover from csi-driver-nfs docs; functionally fine, just misleading)
