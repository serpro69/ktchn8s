...
"""
Post-installation script with some hacky workarounds.
Credits: https://raw.githubusercontent.com/khuedoan/homelab/refs/heads/master/scripts/hacks
...
gitea_host = client.NetworkingV1Api().read_namespaced_ingress(
    'gitea', 'gitea').spec.rules[0].host
gitea_user_secret = client.CoreV1Api().read_namespaced_secret(
    'gitea-admin-secret', 'gitea')
gitea_user = base64.b64decode(
    gitea_user_secret.data['username']).decode("utf-8")
gitea_pass = base64.b64decode(
    gitea_user_secret.data['password']).decode("utf-8")
gitea_url = f"http://{gitea_host}"

kanidm_host = client.NetworkingV1Api().read_namespaced_ingress(
    'kanidm', 'kanidm').spec.rules[0].host
...
def setup_gitea_oauth_app(name: str, redirect_uri: str) -> None:
    ...
    if not any(app['name'] == name for app in current_apps):
        ...
        if resp.status_code == 201:
            ...
            print(
                f"Error creating OAuth application {name} ({resp.status_code})")
            print(resp.content)
            sys.exit(1)


def setup_gitea_auth_with_dex():
    gitea_pod = client.CoreV1Api().list_namespaced_pod(
        namespace='gitea', label_selector='app=gitea').items[0].metadata.name
    client_secret = base64.b64decode(
        client.CoreV1Api().read_namespaced_secret(
            'dex.gitea', 'global-secrets').data['client_secret']
    ).decode("utf-8")
...
def kanidm_login(accounts: list[str]) -> None:
    for account in accounts:
        ...
        cli_login = pexpect.spawn(
            f"kanidm login --url https://{kanidm_host} --name {account}")

        # Wait for the prompt to enter the password since it might not always come as next line
        # For example, this is the output I get when running a different version of client and server:
        #
        # kanidm login --url https://auth.0xbad.cloud --name admin
        # 2025-06-28T19:22:48.978132Z  WARN kanidm_client: Mismatched client and server version - features may not work, or other unforeseen errors may occur. server_version="1.3.3" client_version="1.6.3"
        # Enter password:
        cli_login.expect("Enter password:")
...
def setup_kanidm_oauth_app(name: str, redirect_uri: str) -> None:
    try:
        subprocess.run(
            ["kanidm", "system", "oauth2", "create", "--url",
                f"https://{kanidm_host}", "--name", "idm_admin", name, name, redirect_uri],
            capture_output=True,
            check=True,
        )
    ...
    subprocess.run(
        ["kanidm", "system", "oauth2", "warning-insecure-client-disable-pkce",
            "--url", f"https://{kanidm_host}", "--name", "idm_admin", name],
        capture_output=True,
        check=True,
    )
    ...
    subprocess.run(
        ...
        ["kanidm", "system", "oauth2", "create-scope-map", "--url",
    ...
    client_secret = json.loads(subprocess.run(
        ["kanidm", "system", "oauth2", "show-basic-secret", "--url",
